...
//Oracle Board 테이블에 관한 정보를 가진 모델 객체
...
//		검색인 경우
		...
		try {
			pstmt = boardMain.conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
//			- - - - - - - - - - - - - - - - 쿼리문 수행객체 생성
			rs = pstmt.executeQuery();
//			- - - - - - - - - - - - - - - - 

			rs.last();
			int total = rs.getRow(); // 총 몇 건인지 반환
			data = new String[total][column.length]; // rs를 대체하여 데이터를 담게 될 2차원 배열 생성
...
//	레코드 한 건 넣기(DML)
	public int insert(String title, String writer, String content) {
		...
		sql += "values(seq_board.nextval, '" + title + "', '" + writer + "', '" + content + "')"; // sql+=" values(시퀀스의 다음 값)";
		System.out.println(sql);
		try {
			pstmt = boardMain.conn.prepareStatement(sql);
			// 반환값 int형의 의미 : 이 DML에 의해 영향을 받은 레코드 수를 반환한다.
			// 따라서 insert일 경우 성공시 언제나 1, 실패는 0
...
//	이 메서드를 호출하는 자는 board_id를 넘겨야 한다
	public int delete(int board_id) {
		PreparedStatement pstmt = null;
//		String sql = "delete from board where board_id=유저가 선택한 board_id";
