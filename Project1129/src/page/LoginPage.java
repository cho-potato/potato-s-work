...
public class LoginPage extends JPanel {
	...
	public LoginPage(AppMain appMain) {
		...
		p_form.setPreferredSize(new Dimension(500, 100));
	...
	public void loginChek() {
		...
		try {
			...
			pstmt.setString(2, StringUtil.getConvetredPassword(new String(t_pass.getPassword())));

			rs = pstmt.executeQuery();
			if (rs.next()) { // 레코드가 없으면 회원인증 성공
				JOptionPane.showConfirmDialog(LoginPage.this, "인증성공");
				appMain.loginFlag = true; // 로그인 상태를 true로 전환
			} else {
				JOptionPane.showConfirmDialog(LoginPage.this, "로그인 정보가 올바르지 않슴다");
			}
